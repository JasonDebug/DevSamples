...
--*/
{
    ...
    // Add the session ID
    ULONG sessionId;
    NTSTATUS status = FltGetRequestorSessionId(Data, &sessionId);
    if (NT_SUCCESS(status))
    {
    ...
    {
        recordData->SessionId = (ULONG)-1;
    }

    // Add the user SID
    recordData->UserSidLength = 0;
    recordData->IsImpersonated = FALSE;
    ...
    if (NT_SUCCESS(status))
    {
        recordData->IsImpersonated = TRUE;
    }
    else
    {
        // No impersonation token, fall back to process primary token
    ...
    ULONG returnLen;
    if (NT_SUCCESS(status)) {
        // Query how big the TOKEN_USER buffer is
        ...
        {
            PTOKEN_USER pUserInfo = ExAllocatePoolWithTag(
                PagedPool,
                returnLen,
                'usrX'
            );
            if (pUserInfo) {
                if (NT_SUCCESS(ZwQueryInformationToken(
                    hToken,
                    TokenUser,
                    pUserInfo,
                    returnLen,
                ...
                {
                    ULONG sidLen = RtlLengthSid(pUserInfo->User.Sid);
                    if (sidLen <= MAX_SID_SIZE) {
                        RtlCopyMemory(
                            recordData->UserSidBuffer,
                            pUserInfo->User.Sid,
                            sidLen
                        );
                        recordData->UserSidLength = (USHORT)sidLen;
                    }
                }
                ExFreePoolWithTag(pUserInfo, 'usrX');
            }
        }
        ZwClose(hToken);
    }
