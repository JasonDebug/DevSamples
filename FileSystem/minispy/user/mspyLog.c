...
#pragma warning(pop)
        ...
        for (;;) {
            ...
            // Crude display filter for testing
            //if (wcsstr(pLogRecord->Name, L"TestLog") == NULL) {
            //    // Skip the record
            //    pLogRecord = (PLOG_RECORD)Add2Ptr(pLogRecord, pLogRecord->Length);
            //    continue;
            //}
...
--*/
{
    ...
    // Print new fields
    if (RecordData->UserSidLength > 0) {
        PSID pSid = (PSID)RecordData->UserSidBuffer;
        LPSTR aSid = NULL;
        if (ConvertSidToStringSidA(pSid, &aSid)) {
            fprintf(File, "\tUser SID: %s", aSid);
            LocalFree(aSid);
        }
        else {
            fprintf(File, "\tUser SID: <invalid SID>");
        }
    }
    else {
        fprintf(File, "\tUser SID: <none>");
    }

    if (RecordData->IsImpersonated && RecordData->SessionId == (ULONG)-1) {
        // Likely SMB access not part of a session
        fprintf(File, "\tSession ID: N/A (remote access)");
    }
    else if (RecordData->SessionId == (ULONG)-1) {
        // Likely system access
        fprintf(File, "\tSession ID: N/A (system context)");
    }
    else
    {
        fprintf(File, "\tSession ID: %u", RecordData->SessionId);
    }

    fprintf(File, "\tImpersonated: %s", RecordData->IsImpersonated ? "TRUE" : "FALSE");
    // End new fields
...
--*/
{
    ...
    if (RecordData->UserSidLength > 0) {
        PSID pSid = (PSID)RecordData->UserSidBuffer;
        LPWSTR wSid = NULL;
        if (ConvertSidToStringSidW(pSid, &wSid)) {
            wprintf(L"\n   User SID      : %s\n", wSid);
            LocalFree(wSid);
        }
        else {
            wprintf(L"\n   User SID      : <invalid SID>\n");
        }
    }
    else {
        wprintf(L"   User SID      : <none>\n");
    }
    
    if (RecordData->IsImpersonated && RecordData->SessionId == (ULONG)-1) {
        // Likely SMB access not part of a session
        wprintf(L"   Session ID    : N/A (remote access)\n");
    }
    else if (RecordData->SessionId == (ULONG)-1) {
        // Likely system access
        wprintf(L"   Session ID    : N/A (system context)\n");
    }
    else
    {
    ...
    wprintf(L"   Impersonated  : %s\n", RecordData->IsImpersonated ? L"Yes" : L"No");
    // End new fields
